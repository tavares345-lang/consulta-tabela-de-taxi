...
const LongTripCalculator: React.FC<LongTripCalculatorProps> = ({ 
    ...
    return (
        <div className="space-y-10">
              ...
              <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100 border-l-[12px] border-l-blue-600 animate-in slide-in-from-top-6 duration-500">
                  <div className="flex items-center justify-between mb-8">
                      <div className="flex items-center">
                        ...
                        <h2 className="text-xl font-black text-gray-800 uppercase tracking-widest">Cálculo de Rota Google Maps</h2>
                      </div>
                      {isLoadingDistance && (
                  ...
                  <div className="flex flex-col items-center">
                      ...
                      >
                          ...
                          <span>{isLoadingDistance ? 'Calculando...' : 'Consultar Distância Oficial'}</span>
                      </button>
                          ...
                          <div className="mt-10 w-full max-w-4xl space-y-6">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
                                  <div className="bg-blue-50 p-8 rounded-[40px] border-2 border-blue-100 shadow-inner flex flex-col items-center">
                                      <span className="text-blue-500 text-xs font-black uppercase tracking-widest mb-1 block">GPS (Maps)</span>
                                  ...
                                  <div className={`p-8 rounded-[40px] border-2 flex flex-col items-center justify-center ${matchedSavedTrip ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-200 opacity-60'}`}>
                                      <span className="text-gray-400 text-xs font-black uppercase tracking-widest mb-1 block">Tabela Local</span>
                                      {matchedSavedTrip ? (
                                          ...
                                          <p className="text-sm text-gray-400 font-bold uppercase italic mt-2">Não cadastrado</p>
                                      )}
                                  </div>
                              </div>
                              ...
                              <div className="bg-green-600 rounded-[50px] p-12 shadow-2xl border-8 border-white ring-12 ring-green-50 animate-in zoom-in duration-500 text-center">
                                  <span className="text-white/80 text-sm font-black uppercase tracking-widest mb-2 block">VALOR SUGERIDO (R$ {pricePerKm.toFixed(2)}/KM)</span>
            ...
            {/* Listagem Fixa com zebra striping */}
                ...
                <div className="bg-white rounded-3xl shadow-md border border-gray-100 p-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] text-gray-400 font-bold uppercase mt-0.5 italic">Busca inteligente (ignore acentos)</p>
                            </div>
                        </div>
                        <div className="flex items-center space-x-3">
                            <span className="bg-yellow-100 text-yellow-700 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-tighter">
                                {longTrips.length} {longTrips.length === 1 ? 'Destino' : 'Destinos'}
                            </span>
                            {isFiltered && (
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
                        <div className="xl:col-span-7 relative">
                            ...
                            <input
                                type="text"
                                placeholder="Cidade (ex: Maceio, São Paulo)..."
                                value={searchTerm}
                ...
                <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="hidden md:table-header-group bg-gray-800">
                            <tr>
                                <th className="px-8 py-6 text-sm font-black text-white uppercase tracking-widest">Cidade de Destino</th>
                                <th className="px-8 py-6 text-sm font-black text-white uppercase tracking-widest">Distância</th>
                                <th className="px-8 py-6 text-sm font-black text-white uppercase tracking-widest">Valor Estimado</th>
                                {isAdmin && <th className="px-8 py-6 text-right text-white">Ações</th>}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {longTrips.length > 0 ? longTrips.map((trip, index) => {
                                ...
                                return (
                                    <tr 
                                        key={trip.id} 
                                        className={`block md:table-row transition-colors ${isMatchedGPS ? 'bg-blue-50 ring-2 ring-blue-200 z-10 relative' : (index % 2 === 0 ? 'bg-white' : 'bg-gray-50/80')}`}
                                    >
                                        ...
                                        {isAdmin && (
                                            <td className="px-6 py-4 md:px-8 md:py-6 block md:table-cell text-right">
                                                <div className="flex items-center justify-end space-x-2">
                                                    <button onClick={() => { setEditingTrip(trip); setIsModalOpen(true); }} className="p-2 text-blue-500 hover:bg-blue-100 rounded-xl transition-all"><PencilIcon className="w-5 h-5" /></button>
                                                    <button onClick={() => { if (confirm('Excluir destino?')) onDeleteLongTrip(trip.id); }} className="p-2 text-red-500 hover:bg-red-100 rounded-xl transition-all"><TrashIcon className="w-5 h-5" /></button>
                                                </div>
                                            </td>
                                        )}
                                    </tr>
                                ...
                                <tr>
                                    <td colSpan={isAdmin ? 4 : 3} className="p-24 text-center">
                                        <p className="text-gray-400 text-lg font-medium italic">Nenhum resultado para os filtros aplicados.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
